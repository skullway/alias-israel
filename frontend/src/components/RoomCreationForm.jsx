...
const FAKE_VOCABULARIES = [
  { id: 'nouns', name: 'אוצר מילים כללי' }, // General Vocabulary
  // { id: 'AOLW', name: 'all out llm warfare' },
];
...
const RoomCreationForm = ({ onSubmit }) => {
  ...
  // Add state to manage the inclusion/exclusion of additional words
  const [useAdditionalWordsExclusively, setUseAdditionalWordsExclusively] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    const customWords = roomSettings.wordSource === 'custom' ? roomSettings.customWords.split(',').map(word => word.trim()).filter(word => word.length > 0) : null;
    if (customWords) {
      processedWords = customWords;
      if (!useAdditionalWordsExclusively) {
        roomSettings.wordSource = 'vocabulary'; // Switch to vocabulary to indicate both are used
        roomSettings.selectedVocabularyId = FAKE_VOCABULARIES[0].id; // Default vocabulary
      }
    }
  ...
  // Handler to toggle the inclusion/exclusion of additional words
  const toggleAdditionalWordsMode = () => {
    setUseAdditionalWordsExclusively((prev) => !prev);
  };
          ...
          {/* Vocabulary Option */}
              <div className="flex items-center gap-2">
                ...
                <div className="mt-2">
                  ...
                  onChange={handleInputChange}
                  placeholder="הכנס מילים, מופרדות בפסיקים (לדוגמה: נאגטס, מייפל, 24)"
                  rows="3"
                  ...
                  <input
                    id="exclusive-mode"
                    type="checkbox"
                    checked={useAdditionalWordsExclusively}
                    onChange={toggleAdditionalWordsMode}
                    className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="exclusive-mode" className="ml-2 text-sm text-gray-700 dark:text-gray-200">
                    {useAdditionalWordsExclusively ? 'שימוש בלעדי ברשימה שלי' : 'הוספת מילים נוספות'}
                  </label>
                  <div className="relative group ml-2">
                    <span className="text-gray-500 cursor-pointer">?</span>
                    <div className="absolute bottom-full mb-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded-lg p-2 shadow-lg w-72">
                    כאשר הכפתור מסומן, המשחק ישתמש רק ברשימת המילים שרשמת. כאשר הכפתור לא מסומן, המשחק יוסיף את המילים שרשמת למאגר המילים שנבחר.
                    </div>
                  </div>
                  </div>
                </div>
